...
class InsightService:
    ...
    def generate_insight(
        ...
        try:
            # Build the prompt using the template from POA
...
Implied probability: {old_prob:.1f}% → {new_prob:.1f}% (Δ {delta_pct:+.1f}%){volume_text}{ttr_text}
...
In 3–5 sentences: explain plausible drivers of the move, 2 risks to watch, and a neutral note (not financial advice)."""
            ...
            if message.content and len(message.content) > 0:
                ...
                logger.info(f"Generated insight for market: {market_title[:50]}...")
                return insight_text
            else:
